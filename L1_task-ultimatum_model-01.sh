...
EVDIR=${maindir}/fsl/EVfiles/sub-${sub}/ultimatum/run-0${run}
DATA=${maindir}/fmriprep/fmriprep/sub-${sub}/func/sub-${sub}_task-ultimatum_run-0${run}_bold_space-MNI152NLin2009cAsym_variant-smoothAROMAnonaggr_preproc.nii.gz
MAINOUTPUT=${maindir}/fsl/sub-${sub}/
mkdir -p $MAINOUTPUT
OUTPUT=${MAINOUTPUT}/L1_task-ultimatum_model-01_type-act_run-0${run}
rm -rf ${OUTPUT}.feat

ITEMPLATE=${maindir}/templates/L1_task-ultimatum_model-01_type-act.fsf
OTEMPLATE=${MAINOUTPUT}/L1_task-ultimatum_model-01_type-act_run-0${run}.fsf
