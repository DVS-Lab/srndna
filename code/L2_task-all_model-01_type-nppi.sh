...
# Ultimatum Game Task
NCOPES=13
INPUT1=${MAINOUTPUT}/L1_task-ultimatum_model-01_nppi-${ppi}_run-01_sm-${sm}_variant-${dtype}.feat
INPUT2=${MAINOUTPUT}/L1_task-ultimatum_model-01_nppi-${ppi}_run-02_sm-${sm}_variant-${dtype}.feat

OUTPUT=${MAINOUTPUT}/L2_task-ultimatum_model-01_type-nppi-${ppi}_sm-${sm}_variant-${dtype}
if [ -e ${OUTPUT}.gfeat/cope${NCOPES}.feat/cluster_mask_zstat1.nii.gz ]; then
	echo "skipping existing output"
else
	...
	OTEMPLATE=${MAINOUTPUT}/L2_task-ultimatum_model-01_type-nppi-${ppi}_variant-${dtype}.fsf
	sed -e 's@OUTPUT@'$OUTPUT'@g' \
	-e 's@INPUT1@'$INPUT1'@g' \
	-e 's@INPUT2@'$INPUT2'@g' \
	<$ITEMPLATE> $OTEMPLATE
